...
# Wrapper script to run training with config files for groot
# Usage: ./run.groot.sh configs/groot_default.yaml
...
if [ -z "${CONFIG_FILE}" ]; then
  ...
  echo "Usage: ./run.groot.sh configs/groot_default.yaml"
  echo "   or: CONFIG_FILE=configs/groot_default.yaml ./run.groot.sh"
  exit 1
fi
...
# Run training via Python script using groot-specific docker-compose
docker compose -f docker-compose.groot.yml run --rm \
  -e HF_TOKEN \
  -e WANDB_API_KEY \
  -e BREV_ENV_ID \
  -e BREV_TOKEN \
