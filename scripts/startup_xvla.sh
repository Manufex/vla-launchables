...
export ENV="${ENV:-brev}"
export CONFIG_FILE="${CONFIG_FILE:-configs/xvla_default.yaml}"

# Build once at boot so training starts faster later
docker compose build

echo "============================================================"
echo "VLA Launchable ready (SSH workflow) - X-VLA"
echo ""
echo "After you SSH into the instance, run:"
echo "  cd ${PROJECT_DIR}"
echo "  ./run.sh configs/xvla_default.yaml"
echo ""
echo "Or using just:"
echo "  just train-xvla"
echo ""
...
echo "  nano configs/xvla_default.yaml"
echo "  # or"
echo "  vim configs/xvla_default.yaml"
echo ""
echo "You can modify: dataset.repo_id, steps, batch_size, output_dir, etc."
echo ""
echo "Optional: Create a custom config:"
echo "  cp configs/xvla_default.yaml configs/xvla_custom.yaml"
echo "  nano configs/xvla_custom.yaml"
echo "  ./run.sh configs/xvla_custom.yaml"
echo ""
echo "Auto-delete instance after training (saves costs):"
echo "  1. Edit configs/xvla_default.yaml and set auto_delete.enable: true"
