...
export ENV="${ENV:-brev}"
export CONFIG_FILE="${CONFIG_FILE:-configs/smolvla_default.yaml}"

# Build once at boot so training starts faster later
docker compose build

echo "============================================================"
echo "VLA Launchable ready (SSH workflow) - SmolVLA"
echo ""
echo "After you SSH into the instance, run:"
echo "  cd ${PROJECT_DIR}"
echo "  ./run.sh configs/smolvla_default.yaml"
echo ""
echo "Or using just:"
echo "  just train-smolvla"
echo ""
...
echo "  nano configs/smolvla_default.yaml"
echo "  # or"
echo "  vim configs/smolvla_default.yaml"
echo ""
echo "You can modify: dataset.repo_id, steps, batch_size, output_dir, etc."
echo ""
echo "Optional: Create a custom config:"
echo "  cp configs/smolvla_default.yaml configs/smolvla_custom.yaml"
echo "  nano configs/smolvla_custom.yaml"
echo "  ./run.sh configs/smolvla_custom.yaml"
echo ""
echo "Auto-delete instance after training (saves costs):"
echo "  1. Edit configs/smolvla_default.yaml and set auto_delete.enable: true"
